...
appvars_purge() {
    ...
    APPNAME=${APPNAME^^}
    local APPVARS
    APPVARS=$(grep "^${APPNAME}_" "${SCRIPTPATH}/.data/.env" || true)
    if [[ -z ${APPVARS} ]]; then
        if [[ ${PROMPT:-} == "GUI" ]]; then
            whiptail --fb --clear --title "${APPLICATION_NAME}" --msgbox "${APPNAME} has no variables." 0 0
        else
    ...
    if [[ ${CI:-} == true ]] || run_script 'question_prompt' "${PROMPT:-}" N "Would you like to purge these settings for ${APPNAME}?\\n\\n${APPVARS}"; then
        ...
        sed -i "/^${APPNAME}_/d" "${SCRIPTPATH}/.data/.env" || fatal "Failed to purge ${APPNAME} variables."
    else
