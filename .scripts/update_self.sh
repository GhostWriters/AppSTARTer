...
update_self() {
    ...
    if run_script 'question_prompt' "${PROMPT:-}" Y "Would you like to update ${APPLICATION_NAME} to ${BRANCH} now?"; then
        notice "Updating ${APPLICATION_NAME} to ${BRANCH}."
    else
        notice "${APPLICATION_NAME} will not be updated to ${BRANCH}."
        return 1
    fi
