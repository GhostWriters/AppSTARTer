...
env_sanitize() {
    if grep -q -E '^\w+DIR=~/' "${SCRIPTPATH}/.data/.env"; then
        info "Replacing ~ with ${DETECTED_HOMEDIR} in ${SCRIPTPATH}/.data/.env file."
        sed -i -E "s/^(\w+DIR)=~\//\1=$(sed 's/[&/\]/\\&/g' <<< "${DETECTED_HOMEDIR}")\//g" "${SCRIPTPATH}/.data/.env" | warn "Please verify that ~ is not used in ${SCRIPTPATH}/.data/.env file."
    fi
    ...
    if grep -q 'VPN_ENABLED=true$' "${SCRIPTPATH}/.data/.env" && [[ ${VPN_OVPNDIR} != "" ]]; then
        ...
        find "${APPCONFDIR}" -regex '.*\/openvpn\/.*\.ovpn$' -exec cp {} "${VPN_OVPNDIR}" \; | info "No ovpn files to found."
        find "${APPCONFDIR}" -regex '.*\/openvpn\/.*\.crt$' -exec cp {} "${VPN_OVPNDIR}" \; | info "No crt files to found."
    fi
}
