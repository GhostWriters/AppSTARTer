...
env_set() {
    ...
    local NEW_VAL=${2:-}
    local VAR_VAL
    VAR_VAL=$(grep --color=never "^${SET_VAR}=" "${SCRIPTPATH}/.data/.env") || fatal "Failed to find ${SET_VAR} in ${SCRIPTPATH}/.data/.env"
    ...
    sed -i "s/^${SED_FIND}$/${SED_REPLACE}/" "${SCRIPTPATH}/.data/.env" || fatal "Failed to set ${SED_REPLACE}"
}
