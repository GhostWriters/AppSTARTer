...
menu_backup() {
    ...
    BACKUPOPTS+=("MIN " "Backup your /.data/.env")
    ...
    if [[ ${CI:-} == true ]]; then
        ...
        BACKUPCHOICE=$(whiptail --fb --clear --title "${APPLICATION_NAME}" --menu "What would you like to do?" 0 0 0 "${BACKUPOPTS[@]}" 3>&1 1>&2 2>&3 || echo "Cancel")
    fi
