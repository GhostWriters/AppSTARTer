...
menu_value_prompt() {
    ...
    local DEFAULT_VAL
    if grep -q -Po "^${SET_VAR}=\K.*" "${SCRIPTPATH}/.data/.env.example"; then
        DEFAULT_VAL=$(grep --color=never -Po "^${SET_VAR}=\K.*" "${SCRIPTPATH}/.data/.env.example" || true)
    else
        DEFAULT_VAL=$(run_script 'yml_get' "${APPNAME}" "services.${FILENAME}.labels[com.appstarter.appvars.${VAR_LABEL}]" || true)
    fi
    ...
    if [[ ${CI:-} == true ]]; then
        ...
        SELECTEDVALUE=$(whiptail --fb --clear --title "${APPLICATION_NAME}" --menu "What would you like set for ${SET_VAR}?${VALUEDESCRIPTION}" 0 0 0 "${VALUEOPTIONS[@]}" 3>&1 1>&2 2>&3 || echo "Cancel")
    fi
    ...
    case "${SELECTEDVALUE}" in
        ...
        "Use System ")
            ...
            ;;
        "Enter New ")
            INPUT=$(whiptail --fb --clear --title "${APPLICATION_NAME}" --inputbox "What would you like set for ${SET_VAR}?${VALUEDESCRIPTION}" 0 0 "${CURRENT_VAL}" 3>&1 1>&2 2>&3 || echo "CancelNewEntry")
            ;;
        "Cancel")
        ...
        case "${SET_VAR}" in
            *_ENABLED)
                if [[ ${INPUT} == true ]] || [[ ${INPUT} == false ]]; then
                    ...
                    whiptail --fb --clear --title "${APPLICATION_NAME}" --msgbox "${INPUT} is not true or false. Please try setting ${SET_VAR} again." 0 0
                    menu_value_prompt "${SET_VAR}"
                fi
                ;;
            *_NETWORK_MODE)
                case "${INPUT}" in
                    "" | "bridge" | "host" | "none" | "service:"* | "container:"*)
                        ...
                        ;;
                    *)
                        whiptail --fb --clear --title "${APPLICATION_NAME}" --msgbox "${INPUT} is not a valid network mode. Please try setting ${SET_VAR} again." 0 0
                        menu_value_prompt "${SET_VAR}"
                        ;;
                esac
                ;;
            *_PORT_*)
                if [[ ${INPUT} =~ ^[0-9]+$ ]] || [[ ${INPUT} -ge 0 ]] || [[ ${INPUT} -le 65535 ]]; then
                    ...
                    whiptail --fb --clear --title "${APPLICATION_NAME}" --msgbox "${INPUT} is not a valid port. Please try setting ${SET_VAR} again." 0 0
                    menu_value_prompt "${SET_VAR}"
                fi
                ;;
            *DIR | *DIR_*)
                if [[ ${INPUT} == "/" ]]; then
                    whiptail --fb --clear --title "${APPLICATION_NAME}" --msgbox "Cannot use / for ${SET_VAR}. Please select another folder." 0 0
                    menu_value_prompt "${SET_VAR}"
                elif [[ ${INPUT} == ~* ]]; then
                    ...
                    if run_script 'question_prompt' "${PROMPT:-}" Y "Cannot use the ~ shortcut in ${SET_VAR}. Would you like to use ${CORRECTED_DIR} instead?"; then
                        ...
                        whiptail --fb --clear --title "${APPLICATION_NAME}" --msgbox "Returning to the previous menu to confirm selection." 0 0
                    else
                        whiptail --fb --clear --title "${APPLICATION_NAME}" --msgbox "Cannot use the ~ shortcut in ${SET_VAR}. Please select another folder." 0 0
                    fi
                    menu_value_prompt "${SET_VAR}"
                elif [[ -d ${INPUT} ]]; then
                    run_script 'env_set' "${SET_VAR}" "${INPUT}"
                    ...
                    if run_script 'question_prompt' "${PROMPT:-}" Y "${INPUT} is not a valid path. Would you like to attempt to create it?"; then
                        ...
                        run_script 'set_permissions' "${INPUT}"
                        run_script 'env_set' "${SET_VAR}" "${INPUT}"
                        whiptail --fb --clear --title "${APPLICATION_NAME}" --msgbox "${INPUT} folder was created successfully." 0 0
                    else
                        whiptail --fb --clear --title "${APPLICATION_NAME}" --msgbox "${INPUT} is not a valid path. Please try setting ${SET_VAR} again." 0 0
                        menu_value_prompt "${SET_VAR}"
                    fi
                fi
                ;;
            P[GU]ID)
                if [[ ${INPUT} == "0" ]]; then
                    ...
                    whiptail --fb --clear --title "${APPLICATION_NAME}" --msgbox "${INPUT} is not a valid ${SET_VAR}. Please try setting ${SET_VAR} again." 0 0
                    menu_value_prompt "${SET_VAR}"
                fi
                ;;
            *)
